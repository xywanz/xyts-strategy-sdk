 ...
 public:
  explicit BarLoader(const std::filesystem::path& xydata_dir)
      : bar_dir_(xydata_dir / "bar"),
        ...
        tick_loader_(xydata_dir),
        contract_mgr_(xydata_dir) {}

  std::vector<Bar> LoadBars(const std::string& instr, int64_t period_sec,
  ...
  std::vector<Bar> LoadBars(const std::string& instr, int64_t period_sec,
    ...
    return LoadBars(instr, period_sec, xyu::datetime::date::fromisoformat(begin_date),
                    xyu::datetime::date::fromisoformat(end_date), force_gen);
  }

  std::vector<Bar> LoadMainBars(const std::string& symbol, int64_t period_sec,
                                const xyu::datetime::date& begin_date,
                                const xyu::datetime::date& end_date, bool force_gen = false);
  ...
  std::vector<Bar> LoadMainBars(const std::string& symbol, int64_t period_sec,
                                const std::string& begin_date, const std::string& end_date,
                                bool force_gen = false) {
    return LoadMainBars(symbol, period_sec, xyu::datetime::date::fromisoformat(begin_date),
                        xyu::datetime::date::fromisoformat(end_date), force_gen);
  }

 private:
  void LoadBars(std::vector<Bar>* out, const std::string& instr, int64_t period_sec,
  ...
  void GenerateAndDumpBars(const std::filesystem::path& path, const std::string& instr,
                           int64_t period_sec, const xyu::datetime::date& date);

  std::vector<Bar> GenerateBarsFromTicks(const std::string& instr, int64_t period_sec,
                                         const xyu::datetime::date& date);

  std::filesystem::path bar_dir_;
  TradingCalendar calendar_;
  TickLoader tick_loader_;
