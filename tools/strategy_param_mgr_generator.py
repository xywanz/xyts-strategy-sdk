...
h_head_fmt = '''//Generated from {process}. DO NOT EDIT!
#pragma once
...
update_member_fmt = '''    if (update_json.contains("{name}") && update_json["{name}"].contains("value")) {{
      auto val = update_json["{name}"]["value"].get<{type}>();
      set_{name}(val);
    }}'''
...
init_func_tail = '''  }
'''
init_func_check_param = '''    CheckParm("{name}");'''

update_func_head = '''  //增量更新, 只更新readonly=false的元素
  void Update(const nlohmann::json& update_json) final {'''
update_func_tail = '''
  }
'''

check_func = '''  void CheckParm(const char* param_name) {{
...
def gen_conf_struct_file(conf, output_dir):
    ...
    for name,attr in obj.items():
        ...
        init_func.append(init_func_check_param.format_map(args))
        init_func.append('    {name} = json_params_["{name}"]["value"].get<{type}>();'.format_map(args))
    ...
    class_defs = [h_head_fmt.format(cls_name=cls_name, process=os.path.basename(conf))] +class_funcs + class_members + ["};"]
    class_defs.append('\n')
